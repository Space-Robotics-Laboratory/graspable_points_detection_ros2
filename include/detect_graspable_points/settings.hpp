...
constexpr float palm_diameter_of_finger_joints =
  (std::round(PALM_DIAMETER_OF_FINGER_JOINTS * ratio));
...
constexpr float inside_margin_of_bottom_void_diameter =
  (std::round(INSIDE_MARGIN_OF_BOTTOM_VOID_DIAMETER * ratio));

// Set the gripper-mask size
constexpr float gripper_mask_half_size =
  ((static_cast<float>(palm_diameter_of_finger_joints) / 2) + finger_length + spine_length);
...
constexpr float gripper_mask_top_solid_radius =
  std::round((palm_diameter + margin_of_top_solid_diameter) / 2);
constexpr float gripper_mask_clearance = (std::round(
  (TO_FLOAT(gripper_mask_size) - palm_diameter) / 2 *
  std::tan((90 - OPENING_ANGLE) * (M_PI / 180.0))));
constexpr float gripper_mask_bottom_void_radius = (std::round(
  (palm_diameter / 2) +
  (TO_FLOAT(gripper_mask_height) * std::tan((CLOSING_ANGLE) * (M_PI / 180.0))) -
  inside_margin_of_bottom_void_diameter));
