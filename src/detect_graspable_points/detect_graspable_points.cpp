...
vector<vector<vector<int>>> detect_graspable_points::pcd_voxelize (const pcl::PointCloud<pcl::PointXYZ>  input_pcd, const float cube_size){
  ...
  *point_cloud_2format=point_cloud;
  ...
  pcl::fromPCLPointCloud2 (*cloud_filtered, pcl_after_filtering);
    ...
    // 結果を表示
    std::cout << "*  voxelized_pcd.resize(xmatrix_size, std::vector<std::vector<int>>(ymatrix_size, std::vector<int>(zmatrix_size,0))); \n#Time passed: " << elapsed.count() << " ms" << std::endl;
...
vector<vector<vector<int>>> detect_graspable_points::creategrippermask(GripperParam gripper_param, float voxel_size){
  ...
  if (opening_spine_depth ==1){
    opening_spine_depth =2;
  }
...
sensor_msgs::msg::PointCloud2 detect_graspable_points::combinedAnalysis(
